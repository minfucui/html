<?php
include 'common1.php';
include '../../php/vncfunc.php';
$lang = $_SESSION['lang'];

function updateShares()
{
    global $_SESSION, $cmdPrefix, $uname;
    $s = searchRows('shares', ['status'=>'open', 'type'=>'app']);
    
    if (sizeof($s) == 0 || isset($s['error']))
        return $s;
    $shares = [];
    foreach ($s as $share) {
        $users = explode(' ', $share['targets']);
        if (!in_array($uname, $users))
            continue;
        $ele = [];
        $ele['app'] = $share['pathjobid'];
        $ele['description'] = $share['description'];
        $ele['targets'] = $share['owner'];
        $ele['share'] = '<a href="javascript:access_app_share(\''.$share['id'].
                '\');">'.$_SESSION['lang']['ACCESS'].'</a>';
        $ele['id'] = $share['id'];
        $ele['last_update'] = $share['last_update'];
        $shares[] = $ele;
    }
    foreach ($s as $share) {
        if ($uname != $share['owner'])
            continue;
        $ele = [];
        $ele['app'] = $share['pathjobid'];
        $ele['description'] = $share['description'];
        $ele['targets'] = $share['targets'];
        $ele['share'] = strval($share['nshared']).'个用户已访问';
        $ele['id'] = $share['id'];
        $ele['last_update'] = $share['last_update'];
        $shares[] = $ele;
    }
    return $shares;
}

function validUsers()
{
    global $uname;
    $users = searchRows('users');
    $targets = [];
    if (sizeof($users) > 0 && !isset($users['error']))
        foreach($users as $u)
            if ($u['roles'] != '保密员' &&
                $u['roles'] != '审计员' &&
                $u['roles'] != '管理员' && $u['username'] != $uname)
                $targets[] = $u['username'];
    return $targets;
}

$data = [];
switch($_POST['action']) {
    case 'load':
        $page = yaml_parse_file(basename($_SERVER['PHP_SELF'], '.php').'.yaml');
        if ($page === FALSE)
            fail("Internal error");
        $ret['data'] = $page;
        break;

    case 'update':
        $ret['data']['table'] = updateShares();
        $ret['data']['options'] = validUsers();
        break;
    case 'new_share':
        if ($_POST['data']['jobid'] == '' ||
            $_POST['data']['targets'] == '') {
            fail('ALL_REQUIRED');
        }
        $vusers = validUsers();
        $tusers = explode(' ', $_POST['data']['targets']);
        foreach($tusers as $u)
            if (!in_array($u, $vusers))
                fail($u.'：用户不存在');
        $s = explode(' ', $_POST['data']['share']);
        $pw = $s[0];
        $h = $s[1];
        $sid = $s[2];
        $r = insertARow('shares',['id'=>hexdec(substr(uniqid(), -6)), 'owner'=>$uname,
                             'type'=>'app',
                             'targets'=>$_POST['data']['targets'],
                             'description'=>$_POST['data']['description'],
                             'pathjobid'=>$_POST['data']['jobid'],
                             'pword'=>$pw, 'host'=>$h, 'sid'=>$sid,
                             'status'=>'open',
                             'ntarget'=>sizeof($tusers),
                             'nshared'=>0
                             ]);
        if (isset($r['error']))
            fail($r['error']);
        break;
    case 'getshare':
        if (!isset($_POST['data']))
            fail('Wrong request');
        $e = searchRows('shares', ['id'=>$_POST['data']]);
        if (sizeof($e) != 1 || isset($e['error']))
            fail('cannot find the share');
        $ret['data']['jobid'] = $e[0]['pathjobid'];
        $ret['data']['pword'] = $e[0]['pword'];
        $ret['data']['host'] = $e[0]['host'];
        $ret['data']['sid'] = $e[0]['sid'];
        $ret['data']['uname'] = $uname;
        /* $e[0]['nshared'] = $e[0]['nshared'] + 1;
        modifyARow('shares', ['id'=>$_POST['data']['id']],
                   ['nshared'=>$e[0]['nshared']]); */
        break;
    case 'delete':
        $del = $_POST['data'];
        if (sizeof($del) == 0)
            fail('Wrong request');
        $deleted = 0;
        foreach ($del as $d) {
            $r = searchRows('shares',['id'=>$d]);
            if (sizeof($r) == 0 || isset($r['error']))
                continue;
            if ($r[0]['owner'] != $uname)
                continue;
            deleteARow('shares', ['id'=>$d]);
            $deleted ++;
        }
        if ($deleted == 0)
            fail('只能删除自己的共享');
        break;
    default:
        fail('Wrong request');
        break;
}


echo json_encode($ret);
?>
