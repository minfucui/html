...
$title = 'SkyForm工业仿真云';
$login_logo = '';
$customize = FALSE;  // 是否定制化
if (file_exists('config.yaml') &&
    ($config = yaml_parse_file('config.yaml')) !== FALSE &&  // 配置文件配置了一些用户的home地址，项目作业数，分配资源限制等
    isset($config['title'])) {  // isset函数判断变量是否已设置且不为NULL
    $title = $config['title'];  // 名称替换掉
...
<body>
    ...
    <script type="text/javascript">
        ...
        input.addEventListener("keyup", function(event) {
            if (event.keyCode == 13) { /* press enter to login */  // 回车键登录功能
        ...
        function login() {
            ...
            $.ajax({
                type: "post",
                url: "php/login.php?username=" + valuser + "&password=" + valpwd,  // 跳到登录后台，判断用户是否存在，进行用户验证
