 ...
 */
        ...
        function createTable(tablejson) {  // 与作业表格一致的格式
            var n = " - "
        ...
        function ajRequest(datas) {
            Win10_child._ajax("php/getProjectByApp.php", datas).then(res => {  // 根据应用获得应用实例接口
                tablejson = res;
        ...
        function toProFiles(par) {  // 打开目录
            params = par.id;
            var projPath = document.getElementById(params).rel;
            if (params) {
                var base = projPath.substring(projPath.lastIndexOf('/') + 1);
                base = base.substring(0, base.lastIndexOf('.'));
                Win10_child.openUrl('./folder.php?dir=' + encodeURI(projPath), '文件: ' + base,  // 打开相应地址的文件系统接口
                              [['80%','80%'], ['100px','100px']])
            }
        }

        function toProDetails(par) {  // 查看实例
            params = par.id;
            ...
            if (params) {
                ...
                Win10_child.openUrl("./form.php?projectPath=" + $("#" + params).attr("name"), params);  // 应用与实例共用的表单接口
            }
        }
