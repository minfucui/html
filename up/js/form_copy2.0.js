...
if (pPath && !pPath.includes('/apps/')) {  // 判断是应用还是实例
    gdata = {
        "projectPath": pPath
    }
    ct = "project";
    getForm("php/getProject.php", gdata);  // 获取实例信息接口
    ...
    if (pPath && pPath.includes('/apps'))
        gdata.appPath = pPath;
    ct = "app";
    getForm("php/getApp.php", gdata);  // 获取应用信息接口
    pPath = 'apps/' + cp + '.yaml';  // 应用yaml配置文件地址
}

function getForm(url, data) {  // 打开应用或实例表单
    Win10_child._ajax(url, data).then(res => {
        ...
        if (url == "php/getProject.php") {
            ...
            if (d.dir) { /* multiple apps */
                ...
                appData = d.apps;
                for(i = 0; i < d.apps.length; i++)  // 有多个应用，增加选择功能
                ...
                if (fillForm(d) == 0)  /* no seletable parameters, submit app directly */
                    createOrUpdate(2);  // 不需要填表单的应用，直接运行
            }
        }
        getQSlots();
    }).catch(err => {
...
function getQSlots() {  // 填入cup核数
    var qname = $('#queue').val();
    if (qname == null)
        qname = queueList[0].Name;
    qname = qname.replace('_plus', '');
...
function analyticData(uiParamList, clusterParams, isProject) {  // 分析得到的应用或实例数据，增加集群资源参数设置
    var str = "";
    var i;
    var numSelParams = 0;
    if (clusterParams.size > 0) {
        ...
        for (var [key, value] of clusterParams) {
            if (key == "queue") {
                ...
                if (value == "auto_list" || value.options == "auto_list") {  // 队列没有设定，使用自动列表
                    numSelParams++;
                    for (i = 0; i < queueList.length; i++) {
                ...
                } else {  // 队列没有options属性，则使用全部队列queuesList
    ...
    uiParamList.forEach(param => {
        ...
        param.params.forEach(ele => {
            ...
            if (ele.filed_type == "input" && (ele.value_type == "string" || ele.value_type == '字串行')) {
                ...
                <a style="cursor:pointer;color:#888;font-size:12px" onclick="removeVal(' + ele.id + ',\'' +cleanval + '\')">清 空</a></>';
...
function projectFiles() {  // 数据文件
    var projPath = pPath;
    var base = projPath.substring(projPath.lastIndexOf('/') + 1);
    base = base.substring(0, base.lastIndexOf('.'));
    Win10_child.openUrl('./folder.php?dir=' + encodeURI(projPath), '文件: ' + base,  // 文件目录接口
                   [['80%','80%'], ['100px','100px']])
}
 ...
 * p = 2: run project
 */
function createOrUpdate(p) {  // 1代表创建或更新实例，2代表运行应用或实例
    ...
    if (p == 1) {
        if (ct == "app") {
            ...
            createUpdate("php/createProject.php", par)  // 创建实例接口
        } else {
            if (disa == true) {
                return;
            }
            ...
            par.appName2 = $('#appName').val();
            createUpdate("php/updateProject.php", par)  // 更新实例接口
        ...
        if (ct == "app") {
            par.appName = JSON.parse(localStorage.getItem("cp"));
            par.appName2 = $('#appName').val();
            createOrUpdateAndRun("php/directRunProject.php", par)  // 直接运行应用并提交作业接口
        } else {
            par.projectPath = pPath;
            par.appName = localStorage.getItem("cp");
            par.appName2 = $('#appName').val();
...
function createUpdate(url, par) {
    Win10_child._ajax(url, par).then(res => {
        var d = res;
        if (url == "php/createProject.php") {  // 创建实例接口
            Win10_child.childLayer('创建成功!', 'Created!');  // 弹出提示
            Win10_child.getIndex();
            Win10_child.close();
        } else if (url == "php/updateProject.php") {  // 更新实例接口
...
function createAndRun(params) {  // 保存实例并直接运行
    if (checkFileSelected() == false) {
        return false;
    }
    var clusterParams = {};
    ...
    Win10_child._ajax("php/getProjectByApp.php", datas).then(res => {  // 根据应用名查询应用实例列表
        var d = res;
        var names = [];
        d.forEach(ele => {
            names.push(ele.name)
    ...
    function createpro() {
        Win10_child._ajax("php/createProject.php", par).then(res => {  // 创建实例接口
            var d = res;
            runpro(d);  // 运行实例
        }).catch(err => {
            var em = err.message.split('|');
    ...
    $.ajax({
        ...
        success: function(r) {
            ...
            if (r.code == "0") {
                vncid = r.data;
                var href = {};
                href.url = "./detail.php?id=" + r.data;  // 作业详情界面
...
function runpro(params) {
    ...
    $.ajax({
        type: "post",
        url: "php/getProject.php",  // 获得实例详细信息接口
        contentType: "application/json",
        ...
        $.ajax({
            ...
            success: function(r) {
                ...
                if (r.code == "0") {
                    vncid = r.data;
                    var href = {};
                    href.url = "./detail.php?id=" + r.data;  // 进入作业详情
                    href.title = "作业 " + r.data;
                    Win10_child.open_new_windows(href, true);
                    if (gui) {
                        interval = setInterval(function() {
    ...
    function openVnc() {
        $.ajax({
            type: "post",
            url: "php/aip.php?action=queryJobVncUrl",  // aip命令接口，查询作业的vnc地址
            data: JSON.stringify({
                "jobId": vncid
            }),
            dataType: "json",
...
function getData() {
    ...
    Win10_child._ajax("php/getFiles.php", par).then(res => {  // 获取应用或实例数据文件信息接口
        var d = res;
        files = d.list;
    ...
    files.forEach((ele, index) => {
        if (i == index) {
            if (act == 'path' && ele.type == 'folder') {
                ...
                document.getElementById(fileid).title = fileString
            }
        }
    });
}
...
function removeVal(id,clearVal){
    $('#' + id).val(clearVal)
    $("#"+id).attr("title",clearVal);
}
function nPageBtn() {
    if (valPage < pageNum) {
        valPage++
