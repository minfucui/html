 ...
 */
        Win10.getIndex();
        interval = setInterval("requestajax()", 5000);  // 每5秒执行一次requestajax()，这个函数的目的是查询改用户的作业，保存在data里

        function requestajax() {
            $.ajax({
                type: "post",
                url: "php/queryEvent.php",  // 查询（作业）事件
        ...
        function toWorks() {
            ...
            localStorage.removeItem('getWorksOfProject');  // 移除元素
        ...
        function addIntoFolder(id) {
            ...
            Win10._ajax("php/addProjectToProjectCategory.php", par).then(res => {  // 添加实例到实例类别目录接口
                Win10.getIndex();
            }).catch(err => {
