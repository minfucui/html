...
<body>
    <div class="content-wrapper">
        ...
        <section class="operation buttonGroup">
            ...
            <button class="kill disable-button" onclick="toKill()">结束</button>
        </section>
    ...
    <script type="text/javascript">
        ...
        function getJobs() {
            ...
            setInterval("ajRequest(ajUrl, ajData)", 3000)  // 每3秒运行一次
        }
        ...
        function createTable(tablejson) {
            ...
            $(document).ready(function() {
                var table = $('#jobDataTables').DataTable({  // 将得到的数据填入表格控件中
                    "data": tablejson,
                    "destroy": true,
                    "columns": [{
                        "orderable": false,
        ...
        function toKill() {
            ...
            layer.confirm('确认结束作业' + data.jobId.replace(' ', ',') + '?', {icon: 3, title: '提示'}, function(index){
                ajRequest(url, data);
                layer.close(index);
            });
        }
        ...
        function toVnc(par) {
            ...
            url = vncurlConvert(vncUrl);  // vnc.js里的方法，返回gui界面的url地址
        ...
        function toWorkDetails(par) {
            ...
            //console.log(par.id)
            if (params) {
                Win10_child.openUrl('./detail.php?id=' + params, "作业 " + params);  // 作业详情接口
            }
        }

        function toFiles(path) {
            Win10_child.openUrl('./folder.php?dir=' + encodeURI(path),  // 文件目录系统接口
