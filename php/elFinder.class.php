	...
	public function exec($cmd, $args) {
		...
		if ($this->debug || !empty($args['debug'])) {
			$result['debug'] = array(
				...
				'memory'    => (function_exists('memory_get_peak_usage') ? ceil(memory_get_peak_usage()/1024).'Kb / ' : '').ceil(memory_get_usage()/1024).'Kb / '.ini_get('memory_limit','10G'),
	...
	private function checkChunkedFile($tmpname, $chunk, $cid, $tempDir, $volume = null) {
		if (preg_match('/^(.+)(\.\d+_(\d+))\.part$/s', $chunk, $m)) {
			...
			if ($range && preg_match('/^(\d+),(\d+),(\d+)$/', $range, $ranges)) {
				...
				// check size info
				if ($len != $csize || $start + $len > $size || ($tmpExists && $size != myFilesize($tmp))) {
					return array(self::ERROR_UPLOAD_TEMP, false);
				}
...
//		}
		...
		foreach ($files['name'] as $i => $name) {
			...
			// change mode to be readable by others
			@chmod($tmpname,0644);
