...
abstract class elFinderVolumeDriver {
	...
	protected function configure() {
		...
		if ($path) {
			if (!myFile_Exists($path)) {
				if (@myMkdir($path)) {
					myChmod($path, $this->options['tmbPathMode']);
				} else {
					$path = '';
				}
			} 
			
			if (myIs_Dir($path) && myIs_Readable($path)) {
				$this->tmbPath = $path;
				$this->tmbPathWritable = myIs_Writable($path);
			}
		}
	...
	public function mount(array $opts) {
		...
		if ($this->mimeDetect == 'internal' && !self::$mimetypesLoaded) {
			...
			$file = false;
			if (!empty($this->options['mimefile']) && myFile_Exists($this->options['mimefile'])) {
				$file = $this->options['mimefile'];
			} elseif (myFile_Exists(dirname(__FILE__).DIRECTORY_SEPARATOR.'mime.types')) {
				$file = dirname(__FILE__).DIRECTORY_SEPARATOR.'mime.types';
			} elseif (myFile_Exists(dirname(dirname(__FILE__)).DIRECTORY_SEPARATOR.'mime.types')) {
				$file = dirname(dirname(__FILE__)).DIRECTORY_SEPARATOR.'mime.types';
			}

			if ($file && myFile_Exists($file)) {
				$mimecf = myFile($file);
	...
	public function open($hash) {
		...
		|| $file['mime'] == 'directory') {
	        ...
	        cleanTmp();
		$tmpname=tempnam("/tmp", 'ol'.$GLOBALS['uname']);
		chmod($tmpname, 0666);
		$srcfile=$this->decode($hash);
		myCopy($srcfile,$tmpname);
		chmod($tmpname, 0600);
		return $this->fopenCE($tmpname, 'rb');	
	...
	public function close($fp, $hash) {
		// $filename=$this->decode($hash);
		$this->fcloseCE($fp, $this->decode($hash));
		//$this->fcloseCE($fp, $filename);
		//unlink($filename);
	}
	...
	public function duplicate($hash, $suffix='copy') {
		...
		$path = $this->decode($hash);
		$dir  = $this->dirnameCE($path);
		$name = $this->uniqueName($dir, $this->basenameCE($path), '_'.$suffix.'_');
	...
	public function upload($fp, $dst, $name, $tmpname) {
		...
		$tmpsize = sprintf('%u', myFilesize($tmpname));
	...
	public function resize($hash, $width, $height, $x, $y, $mode = 'resize', $bg = '', $degree = 0, $jpgQuality = null) {
		...
		if (!$work_path || !myIs_Writable($work_path)) {
			if ($work_path && $path !== $work_path && myIs_File($work_path)) {
				@myUnlink($work_path);
			}
			return $this->setError(elFinder::ERROR_PERM_DENIED);
		}
		...
		if ($path !== $work_path) {
			myIs_File($work_path) && @myUnlink($work_path);
		}
	...
	public function getTempPath() {
		$tempPath = null;
		if (isset($this->tmpPath) && $this->tmpPath && myIs_Writable($this->tmpPath)) {
			$tempPath = $this->tmpPath;
		} else if (isset($this->tmp) && $this->tmp && myIs_Writable($this->tmp)) {
			$tempPath = $this->tmp;
		} else if (function_exists('sys_get_temp_dir')) {
			$tempPath = sys_get_temp_dir();
		} else if (isset($this->tmbPath) && $this->tmbPath && myIs_Writable($this->tmbPath)) {
	...
	protected function getTempFile($path = '') {
		...
		if ($tmpdir = $this->getTempPath()) {
			if (!$rmfunc) {
				$rmfunc = create_function('$f', 'myIs_File($f) && @myUnlink($f);');
			}
	...
	protected function gettmb($path, $stat) {
		if ($this->tmbURL && $this->tmbPath) {
			...
			if (myFile_Exists($this->tmbPath.DIRECTORY_SEPARATOR.$name)) {
				return $name;
			}
		}
		return false;
	...
	protected function rmTmb($stat) {
		if ($stat['mime'] === 'directory') {
			...
			myFile_Exists($tmb) && @myUnlink($tmb);
			clearstatcache();
		}
	}
	...
	**/
	...
	protected function getArchivers($use_cache = true) {
		...
		if (function_exists('proc_open')) {
			...
			if ($c == 0) {
				$arcs['create']['application/zip']  = array('cmd' => 'zip', 'argc' => '-r', 'ext' => 'zip');
			}
			unset($o);
	...
	public function rmdirRecursive($dir) {
		if (!myIs_Link($dir) && myIs_Dir($dir)) {
			@myChmod($dir, 0777);
			foreach (array_diff(myScandir($dir), array('.', '..')) as $file) {
				@set_time_limit(30);
				$path = $dir . DIRECTORY_SEPARATOR . $file;
				if (!myIs_Link($dir) && myIs_Dir($path)) {
			...
			return @myRmdir($dir);
		} else if (myIs_File($dir) || myIs_Link($dir)) {
			@myChmod($dir, 0666);
			return @myUnlink($dir);
		}
		return false;
