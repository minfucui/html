...
class elFinderVolumeLocalFileSystem extends elFinderVolumeDriver {
	...
	protected function configure() {
		...
		if (!empty($this->options['tmpPath'])) {
			if ((myIs_Dir($this->options['tmpPath']) || @myMkdir($this->options['tmpPath'], 0755, true)) && myIs_Writable($this->options['tmpPath'])) {
		...
		if (!empty($this->options['quarantine'])) {
			if (myIs_Dir($this->options['quarantine'])) {
				if (myIs_Writable($this->options['quarantine'])) {
					$this->quarantine = $this->options['quarantine'];
				}
				$this->options['quarantine'] = '';
			} else {
				$this->quarantine = $this->_abspath($this->options['quarantine']);
				if ((!myIs_Dir($this->quarantine) && !$this->_mkdir($this->root, $this->options['quarantine'])) || !myIs_Writable($this->quarantine)) {
	...
	protected function _stat($path) {
		...
		if (!myFile_Exists($path) && !myIs_Link($path)) {
			return $stat;
		}
		...
		$stat['isowner'] = false;
		$linkreadable = false;
		if ($path != $this->root && myIs_Link($path)) {
		...
		$size = sprintf('%u', @myFilesize($path));
		$stat['ts'] = myFilemtime($path);
		if ($statOwner) {
			$fstat = myStat($path);
			$uid = $fstat['uid'];
			$gid = $fstat['gid'];
		...
		$stat['read'] = ($linkreadable || myIs_Readable($path))? null : false;
		$stat['write'] = myIs_Writable($path)? null : false;
	...
	protected function getOwnerStat($uid, $gid) {
		...
		if (is_null($phpuid)) {
			if (is_callable('myPosix_Getuid')) {
				$phpuid = myPosix_Getuid();
			} else {
				$phpuid = 0;
			}
		}
		...
		if ($uid) {
			...
			} else if (is_callable('myPosix_Getpwuid')) {
				$pwuid = myPosix_Getpwuid($uid);
		...
		if ($gid) {
			...
			} else if (is_callable('myPosix_Getgrgid')) {
				$grgid = myPosix_Getgrgid($gid);
	...
	protected function _scandir($path) {
		...
		foreach ($dirItr as $file) {
			try {
				...
				$size = sprintf ('%u', myFilesize($fpath));
				// $stat['ts'] = $file->getMTime();
				$stat['ts'] = myFilemtime ($fpath);
				if (!$br) {
					if ($statOwner && !$linkreadable) {
						...
						$stat['perm'] = substr((string)decoct($r['mode']), -4); 
						$stat = array_merge($stat, $this->getOwnerStat($uid, $gid));
					}
					
					//logical rights first
					//$stat['read'] = ($linkreadable || $file->isReadable())? null : false;
					$stat['read'] = ($linkreadable || myIs_Readable($fpath))? null : false;
					// $stat['write'] = $file->isWritable()? null : false;
					$stat['write'] = myIs_Writable($fpath)? null : false;
	...
	protected function _mkdir($path, $name) {
		...
		if (@myMkdir($path)) {
			@myChmod($path, $this->options['dirMode']);
			clearstatcache();
			return $path;
		}
		return false;
	...
	protected function _mkfile($path, $name) {
		...
		//	@fclose($fp);
	 	if (myTouch($path)) {
			@myChmod($path, $this->options['fileMode']);
			clearstatcache();
			return $path;
		}
		return false;
	...
	protected function _symlink($source, $targetDir, $name) {
		return @mySymlink($source, $this->_joinPath($targetDir, $name));
	}
	...
	protected function _copy($source, $targetDir, $name) {
		$ret = myCopy($source, $this->_joinPath($targetDir, $name));
		$ret && clearstatcache();
		return $ret;
	}
	...
	protected function _move($source, $targetDir, $name) {
		...
		$ret = @myRename($source, $target) ? $target : false;
		$ret && clearstatcache();
		return $ret;
	}
	...
	protected function _getContents($path) {
		return myFile_Get_Contents($path);
	}
	...
	protected function _filePutContents($path, $content) {
		if (@myFile_Put_Contents($path, $content, LOCK_EX) !== false) {
			clearstatcache();
			return true;
		}
		return false;
	...
	protected function _findSymlinks($path) {
		...
		if (myIs_Dir($path)) {
			foreach (myScandir($path) as $name) {
				if ($name != '.' && $name != '..') {
					$p = $path.DIRECTORY_SEPARATOR.$name;
					if (myIs_Link($p) || !$this->nameAccepted($name)
						||
					...
					if (myIs_Dir($p) && $this->_findSymlinks($p)) {
						return true;
					} elseif (myIs_File($p)) {
						$this->archiveSize += sprintf('%u', myFilesize($p));
					}
				}
			}
		} else {
			
			$this->archiveSize += sprintf('%u', myFilesize($path));
		}
	...
	protected function _extract($path, $arc) {
		...
		if ($this->quarantine) {
			...
			$dir     = myDirname($path).DIRECTORY_SEPARATOR.md5(myBasename($path).mt_rand());
			$archive = $dir.DIRECTORY_SEPARATOR.myBasename($path);
			...
			register_shutdown_function(array($this, 'rmdirRecursive'), myRealpath($dir));
			
			myChmod($dir, 0777);
			
			// copy in quarantine
			if (!myCopy($path, $archive)) {
				return false;
			}
			
			// extract in quarantine
			myUnpackArchive($archive, $arc);
			
			// get files list
			$ls = array();
			foreach (myScandir($dir) as $i => $name) {
				if ($name != '.' && $name != '..') {
					$ls[] = $name;
				}
			}
			...
			if (($extractTo === 'auto' || !$extractTo) && count($ls) === 1 && myIs_File($src)) {
				$name = $ls[0];
				...
				$test = myDirname($path).DIRECTORY_SEPARATOR.$name;
				if (myFile_Exists($test) || myIs_Link($test)) {
					$name = $this->uniqueName(myDirname($path), $name, '-', false);
				}
			}
			
			if ($name !== '') {
				$result  = myDirname($path).DIRECTORY_SEPARATOR.$name;

				if (! @myRename($src, $result)) {
					$this->delTree($dir);
					return false;
				}
			} else {
				$dstDir = myDirname($path);
				$res = false;
				$result = array();
				foreach($ls as $name) {
					...
					if (@myRename($dir.DIRECTORY_SEPARATOR.$name, $target)) {
						$result[] = $target;
					}
				}
				if (!$result) {
			...
			myIs_Dir($dir) && $this->delTree($dir);
			
			return (is_array($result) || myFile_Exists($result)) ? $result : false;
		}
	}
	...
	protected function _archive($dir, $files, $name, $arc) {
		return myMakeArchive($dir, $files, $name, $arc); 
	}
