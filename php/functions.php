...
$runas='source /var/www/html/env.sh;export LANG=en_US.UTF-8;export OLWD='.$pword.';/var/www/html/cmd/runas '.$uname.' ';
$file_exists='1 ';
$mkdir='2 ';
$chmod='3 ';
$is_dir='4 ';
...
function myRename($old, $new){
    // $f=str_replace(" ","_", $new);
    exec($GLOBALS['runas'].$GLOBALS['rename'].
         escapeshellarg($old).' '.escapeshellarg($new), $r, $eno);
    if ($eno!=0) {
        // Need to symbolic link from /usr/bin/mv to /opt/skyformai/bin
        // as runas can only run command from /opt/skyformai/bin
        exec($GLOBALS['runas'].'mv -f '.escapeshellarg($old).' '.
             escapeshellarg($new), $r, $eno);
        if ($eno != 0)
            return FALSE;
    }
    return TRUE;
...
function myMakeArchive($dir, $files, $name, $arc) {
    $argc = (strpos($arc['cmd'], 'zip') !== FALSE) ? $arc['argc'] : substr($arc['argc'], 1);			// tar operations do not need to be preceded by the - symbol
    if ($argc == '-r9')
        $argc = '-r';
...
function cleanTmp() {
    ...
    $now = time();
    foreach ($allF as $f) {
        if (strpos($f, 'ol'.$uname) === FALSE)
            continue;
        $fpath = '/tmp/'.$f;
        if ($now - fileatime($fpath) > 3600)
            unlink($fpath);
    }
}
?>
